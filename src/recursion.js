...
var countValuesInObj = function(obj, value) {
  ...
  for (key in obj) {
    ...
    if (typeof obj[key] === 'object') {
      thisCount += countValuesInObj(obj[key], value);
    }
  }
  return thisCount;
};
...
var fibonacci = function(n) {
  ...
  let thisEl = prev[prev.length - 2] + prev[prev.length - 1];

  return prev.concat(thisEl);
};
...
var flatten = function(array) {
  ...
  if (!Array.isArray(subArray)) {
    ...
    thisResult = flatten(subArray);
  }
  return thisResult.concat(flatten(array.slice(1)));
};
...
var augmentElements = function(array, aug) {
  ...
  let thisEl = [array[0].concat(aug)];
  return thisEl.concat(augmentElements(array.slice(1), aug));
};
...
var alternateSign = function(array) {
  ...
  let el1 = Math.abs(array[0]);
  let el2 = Math.abs(array[1]);
  if (array.length === 1) {
    return [el1];
  }

  return [el1, -el2].concat(alternateSign(array.slice(2)));
};
...
var numToText = function(str) {
  let words = str.split(' ');
  let writtenWords = ['zero', 'one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine'];

  if (words[0] > 0 && words[0] < 10) {
    words[0] = writtenWords[words[0]];
  }
  ...
  return [words[0], numToText(remainingWords)].join(' ');
};
